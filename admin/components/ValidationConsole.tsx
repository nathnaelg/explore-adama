...
const ValidationConsole: React.FC<ValidationConsoleProps> = ({
  ...
  const handleValidate = async (codeOverride?: string) => {
    ...
    try {
      ...
      setScanResult({
        ...
        ticket: detailedTicket
          ? {
              ...
              holderName:
                detailedTicket.user?.email || detailedTicket.userId || "Guest",
              date: detailedTicket.event?.date
                ? new Date(detailedTicket.event.date).toLocaleDateString()
                : "N/A",
            }
          : {
              id: res.ticketId || "N/A",
      ...
      const msg =
        error.response?.data?.message || "Invalid token or server error.";
  ...
  return (
    <div className={cn("space-y-6", className)}>
      <Card className="border-none shadow-2xl bg-white dark:bg-zinc-900 overflow-hidden rounded-[2.5rem]">
        <CardHeader className="bg-gray-50/50 dark:bg-zinc-950/50 border-b border-gray-100 dark:border-zinc-800 p-8">
          <CardTitle className="flex items-center gap-2 font-black text-xl">
            <ScanLine className="text-blue-500" /> Ticket Validation
          </CardTitle>
          <CardDescription className="text-xs font-bold uppercase tracking-widest text-gray-400">
            Check Ticket Status
          </CardDescription>
        </CardHeader>
        <CardContent className="p-8">
          <div className="space-y-6">
            ...
            <Button
              className="w-full h-16 text-lg font-black uppercase tracking-widest shadow-xl shadow-blue-500/20 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white"
              size="lg"
              ...
              <div
                className={cn(
                  ...
                  scanResult.status === "success"
                    ? "bg-emerald-50/50 border-emerald-200 dark:bg-emerald-900/10 dark:border-emerald-900/20"
                    : scanResult.status === "info"
                      ? "bg-blue-50/50 border-blue-200 dark:bg-blue-900/10 dark:border-blue-900/20"
              ...
              >
                ...
                {scanResult.ticket && scanResult.ticket.id && (
                  <div className="space-y-2 text-[10px] pt-2 border-t border-gray-100 dark:border-zinc-800">
                    ...
                    <div className="flex justify-between pb-1">
                      <span>Status:</span>
                      <span className="font-black text-blue-500 uppercase">
          ...
          <Card className="p-6 rounded-[2rem] bg-emerald-50/30 dark:bg-emerald-900/10 border-none">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xl font-black">{totalCount} Tickets</p>
              </div>
            </div>
          </Card>
        </div>
      ...
      <Dialog open={isScannerOpen} onOpenChange={setIsScannerOpen}>
        <DialogContent
          className="sm:max-w-[500px] p-0 overflow-hidden bg-black rounded-[2.5rem] border-none"
          onClose={stopScanner}
        >
