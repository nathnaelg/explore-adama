...
import { 
  ...
  FileText, MessageSquare, Trash2, Clock, Loader2,
...
const ContentModeration: React.FC<ContentModerationProps> = ({ isDarkMode, searchTerm = '' }) => {
  const [activeTypeFilter, setActiveTypeFilter] = useState<'ALL' | 'BLOG' | 'REVIEW'>('ALL');
  ...
  const fetchData = async () => {
    ...
    try {
        ...
        const [blogRes, reviewRes] = await Promise.all([
            api.get<{ items: any[] }>(`/blog?${statusParam}&page=1&limit=50`).catch(() => ({ items: [] })),
            api.get<{ items: any[] }>(`/reviews?${statusParam}&page=1&limit=50`).catch(() => ({ items: [] }))
        ]);

        const blogItems: UnifiedItem[] = (blogRes.items || []).map((post: any) => ({
            id: post.id,
            type: 'BLOG',
            title: post.title,
            ...
            author: {
                ...
                avatar: post.author?.profile?.avatar || `https://ui-avatars.com/api/?name=${post.author?.email || 'U'}`,
                role: 'EDITOR'
            },
        ...
        const reviewItems: UnifiedItem[] = (reviewRes.items || []).map((review: any) => ({
            id: review.id,
            type: 'REVIEW',
            title: review.placeId ? 'Place Review' : (review.eventId ? 'Event Review' : 'General Review'),
            content: review.comment,
            author: {
                id: review.userId,
                name: review.user?.profile?.name || review.user?.email || 'Unknown User',
  ...
  const filteredItems = items.filter(item => {
    ...
    const localStatusMatch = statusFilter === 'ALL' || 
                             item.status === statusFilter || 
                             (statusFilter === 'APPROVED' && item.status === 'PUBLISHED');

    return matchesSearch && dateMatch && typeMatch && localStatusMatch;
  });
  ...
  const handleStatusChange = async (id: string, newStatus: string) => {
    ...
    try {
        ...
        showFeedback('success', `Content ${newStatus.toLowerCase()} successfully.`);
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">Review and approve user-submitted content.</p>
        </div>
        <div className="flex flex-col md:flex-row items-center gap-3 w-full md:w-auto">
          <Select value={timeRange} onChange={(e) => handleTimeRangeChange(e.target.value)} className="w-full md:w-[130px] font-bold text-xs uppercase tracking-widest rounded-xl">
          ...
          <div className="flex flex-wrap items-center gap-3">
              <span className="text-xs font-bold text-gray-400 uppercase tracking-widest mr-2">Filter Item Type & Status:</span>
              <div className="bg-white dark:bg-zinc-900 p-1 rounded-xl flex gap-1 border border-gray-100 dark:border-zinc-800 shadow-sm overflow-x-auto">
                <button
                  ...
                  className={cn(
                    ...
                    activeTypeFilter === 'ALL' ? "bg-blue-500 text-white shadow-md" : "text-gray-500 hover:bg-gray-100 dark:hover:bg-zinc-800"
                  )}
                >
                  All Types
                </button>
                <button
                  ...
                  className={cn(
                    "px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest flex items-center gap-2 transition-all whitespace-nowrap",
                    activeTypeFilter === 'BLOG' ? "bg-blue-500 text-white shadow-md" : "text-gray-500 hover:bg-gray-100 dark:hover:bg-zinc-800"
                  )}
                >
                  <FileText size={14} /> Blogs
                </button>
                <button
                  onClick={() => setActiveTypeFilter('REVIEW')}
                  className={cn(
                    "px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest flex items-center gap-2 transition-all whitespace-nowrap",
                    activeTypeFilter === 'REVIEW' ? "bg-blue-500 text-white shadow-md" : "text-gray-500 hover:bg-gray-100 dark:hover:bg-zinc-800"
                  )}
                >
              ...
              <div className="flex bg-white dark:bg-zinc-900 p-1 rounded-xl border border-gray-100 dark:border-zinc-800 shadow-sm overflow-x-auto">
                  <button
                    ...
                    className={cn(
                      "px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest flex items-center gap-2 transition-all whitespace-nowrap",
                      statusFilter === 'PENDING' ? "bg-yellow-500 text-white shadow-md" : "text-gray-500 hover:bg-gray-100 dark:hover:bg-zinc-800"
                    )}
                  >
                  ...
                  <button
                    ...
                    className={cn(
                      "px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest flex items-center gap-2 transition-all whitespace-nowrap",
                      statusFilter === 'APPROVED' ? "bg-green-600 text-white shadow-md" : "text-gray-500 hover:bg-gray-100 dark:hover:bg-zinc-800"
                    )}
                  >
                  ...
                  <button
                    ...
                    className={cn(
                      "px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest flex items-center gap-2 transition-all whitespace-nowrap",
                      statusFilter === 'REJECTED' ? "bg-red-600 text-white shadow-md" : "text-gray-500 hover:bg-gray-100 dark:hover:bg-zinc-800"
                    )}
                  >
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      statusFilter === 'ALL' ? "bg-gray-600 text-white shadow-md" : "text-gray-500 hover:bg-gray-100 dark:hover:bg-zinc-800"
                    )}
                  >
                    SHOW ALL
                  </button>
      ...
      <div className="grid grid-cols-1 gap-6">
          {isLoading ? (
             ...
             <div className="text-center py-20 text-gray-400 bg-white dark:bg-zinc-900 rounded-3xl border border-gray-100 dark:border-zinc-800">
                ...
                <p>{searchTerm ? `No items found matching "${searchTerm}"` : `No items in ${statusFilter.toLowerCase()} queue.`}</p>
            </div>
            ...
            >
              <div className="flex flex-col md:flex-row gap-6 w-full">
                  <div className="shrink-0">
                      {item.media ? (
                          <img src={item.media} alt="" className="w-full md:w-48 h-48 md:h-full rounded-2xl object-cover shadow-sm" />
                      ) : (
                          <div className={cn(
                              "w-full md:w-32 h-32 rounded-2xl flex flex-col items-center justify-center gap-2",
                              item.type === 'BLOG' ? "bg-purple-50 text-purple-500 dark:bg-purple-900/20" : "bg-blue-50 text-blue-500 dark:bg-blue-900/20"
                          )}>
                              {item.type === 'BLOG' ? <FileText size={32} /> : <MessageSquare size={32} />}
                  ...
                  <div className="flex-1 flex flex-col min-w-0">
                    ...
                    <div className="flex items-center gap-2 mb-4">
                        <img src={item.author.avatar || `https://ui-avatars.com/api/?name=${item.author.name}`} alt="" className="w-6 h-6 rounded-full object-cover" />
                        <span className="text-sm font-medium text-gray-600 dark:text-zinc-300">by {item.author.name}</span>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-3 pt-4 border-t border-gray-100 dark:border-zinc-800 mt-auto">
                        <Button 
                            ...
                            disabled={processingId === item.id || item.status === 'APPROVED' || item.status === 'PUBLISHED'}
                            onClick={() => handleStatusChange(item.id, 'APPROVED')}
                        >
                            {processingId === item.id ? <Loader2 size={18} className="animate-spin"/> : <Check size={18} />} 
                            {item.status === 'APPROVED' || item.status === 'PUBLISHED' ? 'Approved' : 'Approve'}
                        </Button>
