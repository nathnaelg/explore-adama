  ...
  getStats: async (range: string = "month"): Promise<any> => {
    try {
      const [
        ...
        coreClient
          .get("/bookings", { params: { perPage: 1000 } })
          .catch(() => ({ data: [] })),
        coreClient
          .get("/users", { params: { perPage: 1000 } })
          .catch(() => ({ data: [] })),
        coreClient
          .get("/interactions", { params: { perPage: 1000 } })
          .catch(() => ({ data: [] })),
        coreClient
          .get("/places", { params: { perPage: 1000 } })
          .catch(() => ({ data: [] })),
        coreClient
          .get("/events", { params: { perPage: 1000 } })
          .catch(() => ({ data: [] })),
        coreClient
          .get("/blog", { params: { limit: 1000 } })
          .catch(() => ({ items: [] })),
      ...
      const bannedUsers = users.filter(
        (u: any) =>
          u &&
          (u.banned === true || String(u.banned) === "true" || u.banned === 1),
      );
      const bannedCount = bannedUsers.length;

      const userAnalytics = {
        total: users.length,
        newCount: users.filter(
          (u: any) => u && u.createdAt && new Date(u.createdAt) >= cutoff,
        ).length,
      ...
      const totalCapacity = events.reduce(
        (sum: number, e: any) => sum + (Number(e?.capacity) || 0),
        0,
      );
      const totalIssued = bookings.reduce(
        (sum: number, b: any) => sum + (Number(b?.quantity) || 1),
        0,
      );
      ...
      const eventsAnalytics = {
        ...
        upcoming: events.filter(
          (e: any) => e && e.date && new Date(e.date) >= new Date(),
        ).length,
        totalCapacity,
        totalIssued,
        occupancyRate:
          totalCapacity > 0
            ? Math.round((totalIssued / totalCapacity) * 100)
            : 0,
        createdTrend: [] as any[],
      ...
      eventsAnalytics.createdTrend = months.map((m) => ({
        name: m,
        value: currentTrend[m] || 0,
      }));
      ...
      return {
        ...
        bookings: {
          todayCount: bookings.filter(
            (b: any) =>
              b && b.createdAt && new Date(b.createdAt) >= startOfToday,
          ).length,
        },
  ...
  getGrowthTrends: async (range: string = "month"): Promise<any[]> => {
    try {
      ...
      const grouped: Record<
        string,
        { bookings: number; users: number; tickets: number }
      > = {};

      users
        .filter((u: any) => u && u.createdAt && new Date(u.createdAt) >= cutoff)
        .forEach((u: any) => {
          const label = new Date(u.createdAt).toLocaleDateString("default", {
            month: "short",
            day: "numeric",
          });
          if (!grouped[label])
            grouped[label] = { bookings: 0, users: 0, tickets: 0 };
          grouped[label].users += 1;
        });

      bookings
        .filter((b: any) => b && b.createdAt && new Date(b.createdAt) >= cutoff)
        .forEach((b: any) => {
          const label = new Date(b.createdAt).toLocaleDateString("default", {
            month: "short",
            day: "numeric",
          });
          if (!grouped[label])
          ...
          grouped[label].tickets += Number(b.quantity) || 1;
        });
