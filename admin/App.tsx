...
const App: React.FC = () => {
  const { isDarkMode, toggleTheme, logout, isAuthenticated, isLoading, user } =
    useAuth();

  const [activeTab, setActiveTab] = React.useState("dashboard");
  const [isSidebarCollapsed, setIsSidebarCollapsed] = React.useState(false);
  const [isMobileMenuOpen, setIsMobileMenuOpen] = React.useState(false);
  const [authMode, setAuthMode] = React.useState<"login" | "forgot-password">(
    "login",
  );
        ...
        <div className="max-w-md space-y-4 animate-in slide-in-from-bottom-4 duration-700">
          ...
          <p className="text-gray-500 dark:text-zinc-400 text-lg leading-relaxed">
            ...
            <span className="block mt-2 font-bold text-red-600 dark:text-red-400">
              All dashboard privileges are revoked.
            </span>
          </p>
          <div className="pt-8 flex flex-col gap-3">
            <Button
              ...
              size="lg"
              onClick={() => logout()}
              className="h-14 text-lg font-bold rounded-2xl gap-3 shadow-xl"
            >
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case "places":
        return (
          <PlacesMap isDarkMode={isDarkMode} searchTerm={debouncedSearch} />
        );
      case "content":
        return (
          <ContentManagement
    ...
    <div className="min-h-screen w-full flex relative bg-slate-50 dark:bg-black text-gray-900 dark:text-zinc-100 transition-colors duration-300 font-sans">
      <div
        className={cn(
          ...
          isSidebarCollapsed ? "md:w-28" : "md:w-80",
        )}
      >
        <Sidebar
          activeTab={activeTab}
          setActiveTab={(tab) =>
            handleNavigate(tab === "dashboard" ? "/" : `/${tab}`)
          }
          isDarkMode={isDarkMode}
