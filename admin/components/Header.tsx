...
import { 
  Search, Bell, Moon, Sun, Menu, LogOut,
  MapPin, Calendar, User as UserIcon, Loader2, X, ChevronRight
} from 'lucide-react';
...
const Header: React.FC<HeaderProps> = ({ 
  ...
  // Search State
  const [quickResults, setQuickResults] = useState<{users: any[], places: any[], events: any[]} | null>(null);
    ...
    const fetchResults = async () => {
        ...
        try {
            ...
            const [usersRes, placesRes, eventsRes] = await Promise.all([
                api.get<{data: any[]}>('/users?perPage=100').catch(() => ({ data: [] })),
                api.get<{data: any[]}>('/places?perPage=100').catch(() => ({ data: [] })),
                api.get<{data: any[]}>('/events?perPage=100').catch(() => ({ data: [] }))
            ]);
            ...
            const filteredUsers = rawUsers.filter((u: any) => 
                ...
                (u.profile?.name && u.profile.name.toLowerCase().includes(lowerQ)) ||
            ...
            const filteredPlaces = rawPlaces.filter((p: any) => 
                ...
                (p.category && (typeof p.category === 'string' ? p.category : p.category.name).toLowerCase().includes(lowerQ))
            ).slice(0, 3);
            ...
            const hasResults = filteredUsers.length + filteredPlaces.length + filteredEvents.length > 0;
            ...
            if (hasResults) {
                ...
                const normalizeUser = (u: any) => ({
                    ...
                    name: (typeof u.profile === 'object' ? u.profile?.name : u.name) || u.email || 'User',
                    avatar: (typeof u.profile === 'object' ? u.profile?.avatar : u.avatar) || null,
                    email: u.email || ''
                });
                ...
                const normalizePlace = (p: any) => ({
                    ...
                    image: Array.isArray(p.images) && p.images[0] ? p.images[0].url : (p.image || '')
                });
                ...
                const normalizeEvent = (e: any) => ({
                    ...
                    location: typeof e.place === 'object' ? e.place?.name : (e.location || 'Unknown Location')
                });
  ...
  const userName = activeUser?.profile?.name || activeUser?.name || activeUser?.email?.split('@')[0] || 'Admin';
  const userRole = activeUser?.role || 'ADMIN';
  const userEmail = activeUser?.email || '';
  const userAvatar = activeUser?.profile?.avatar || activeUser?.avatar || `https://ui-avatars.com/api/?name=${userName}&background=random`;
  ...
  return (
    <>
      {(isProfileOpen || isNotifOpen) && <Backdrop onClick={() => { setIsProfileOpen(false); setIsNotifOpen(false); }} />}
      ...
      <header className={cn(
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <div className="hidden md:block">
              ...
              <p className="text-xs text-gray-400 mt-0.5">Welcome back, {userName}</p>
          </div>
        </div>
        ...
        >
          <Search className="absolute left-3 md:left-4 text-gray-400 pointer-events-none" size={18} />
          <Input 
            type="text" 
          ...
          {isSearchFocused && searchValue && searchValue.length > 1 && quickResults && (
              <div className="absolute top-full left-0 w-full mt-2 bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl border border-gray-100 dark:border-zinc-800 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
                      ...
                      {quickResults.users.length > 0 && (
                          <div className="p-2">
                              ...
                              {quickResults.users.map(u => (
                                  <div 
                                    key={u.id} 
                                    onClick={() => handleSearchResultClick(`/users?q=${encodeURIComponent(u.email)}`)}
                                  ...
                                  >
                                      <img src={u.avatar || `https://ui-avatars.com/api/?name=${u.name}`} className="w-8 h-8 rounded-full object-cover bg-gray-200" alt="" />
                                      <div className="flex-1 min-w-0">
                                          <div className="text-sm font-semibold truncate text-gray-900 dark:text-zinc-100">{u.name}</div>
                      ...
                      {quickResults.places.length > 0 && (
                          <div className="p-2 border-t border-gray-100 dark:border-zinc-800">
                              ...
                              {quickResults.places.map(p => (
                                  <div 
                                    key={p.id} 
                                    onClick={() => handleSearchResultClick(`/places?q=${encodeURIComponent(p.name)}`)}
                                    className="flex items-center gap-3 p-2 hover:bg-gray-50 dark:hover:bg-zinc-800 rounded-xl cursor-pointer transition-colors"
                                  >
                                      <img src={p.image || ''} className="w-8 h-8 rounded-lg object-cover bg-gray-200" alt="" />
                                      <div className="flex-1 min-w-0">
                                          <div className="text-sm font-semibold truncate text-gray-900 dark:text-zinc-100">{p.name}</div>
                      ...
                      {quickResults.events.length > 0 && (
                          <div className="p-2 border-t border-gray-100 dark:border-zinc-800">
                              ...
                              {quickResults.events.map(e => (
                                  <div 
                                    key={e.id} 
                                    onClick={() => handleSearchResultClick(`/events?q=${encodeURIComponent(e.title)}`)}
                                    className="flex items-center gap-3 p-2 hover:bg-gray-50 dark:hover:bg-zinc-800 rounded-xl cursor-pointer transition-colors"
                                  >
                                      ...
                                      <div className="flex-1 min-w-0">
                                          <div className="text-sm font-semibold truncate text-gray-900 dark:text-zinc-100">{e.title}</div>
                                          <div className="text-xs text-gray-500 truncate">{e.location}</div>
                                      </div>
                                      <ChevronRight size={14} className="text-gray-300" />
                                  </div>
                              ))}
                  ...
                  <div className="p-2 border-t border-gray-100 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-950/50 text-center">
                      <button 
                        onClick={() => handleSearchResultClick(`/?q=${encodeURIComponent(searchValue || '')}`)}
            ...
            {isNotifOpen && (
               <div className="absolute top-full right-0 mt-2 w-80 md:w-96 bg-white dark:bg-zinc-900 rounded-2xl shadow-xl border border-gray-100 dark:border-zinc-800 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
                  <div className="p-4 border-b border-gray-100 dark:border-zinc-800 flex items-center justify-between bg-gray-50/50 dark:bg-zinc-950/50">
                      ...
                      <button type="button" onClick={handleMarkAllRead} className="text-xs text-blue-500">Mark all read</button>
                  </div>
                  <div className="max-h-[350px] overflow-y-auto scrollbar-thin">
                      {notifications.length === 0 ? (
                          ...
                          notifications.map(n => (
                              <div key={n.id} className={cn("p-4 border-b hover:bg-gray-50 dark:hover:bg-zinc-800/50 cursor-pointer", !n.isRead && "bg-blue-50/50 dark:bg-blue-900/10")} onClick={() => handleNotificationClick(n)}>
                                  <div className="flex justify-between items-start mb-1">
                                      <h4 className="text-sm font-semibold">{n.title}</h4>
                                      <span className="text-[10px] text-gray-400">{n.time}</span>
                                  </div>
                                  <p className="text-xs text-gray-500 dark:text-zinc-400 line-clamp-2">{n.message}</p>
                              </div>
                          ))
                      )}
                  </div>
                  {notifications.length > 0 && (
                      <div className="p-2 border-t text-center">
                           <button onClick={handleClearAll} className="text-xs text-red-500">Clear all</button>
                      </div>
                  )}
               </div>
            )}
          ...
          <div className="relative pl-2 md:pl-4 md:border-l border-gray-200 dark:border-zinc-800 md:ml-2">
            ...
            >
              <div className="text-right hidden lg:block">
                ...
                <p className="text-[10px] font-medium text-gray-400 mt-1 uppercase tracking-wide">{userRole}</p>
              </div>
              <img src={userAvatar} alt="Profile" className="w-8 h-8 md:w-10 md:h-10 rounded-xl object-cover border-2 border-white dark:border-zinc-800 shadow-md bg-gray-100" />
            </button>
            ...
            {isProfileOpen && (
                <div className="absolute top-full right-0 mt-3 w-80 bg-white dark:bg-zinc-900 rounded-2xl shadow-xl border border-gray-100 dark:border-zinc-800 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
                    <div className="p-6 text-center border-b border-gray-100 dark:border-zinc-800">
                        <img src={userAvatar} alt="Profile" className="w-20 h-20 rounded-full mx-auto mb-3 object-cover border-4 border-gray-50 dark:border-zinc-800 bg-gray-100" />
                    ...
                    <div className="p-4 space-y-3 bg-gray-50/50 dark:bg-zinc-950/50">
                        <div className="grid grid-cols-2 gap-4 text-sm">
                             <div>
                                 ...
                                 <span className="font-medium text-gray-900 dark:text-zinc-300">{userPhone}</span>
                             </div>
                             <div>
                                 <span className="text-xs text-gray-400 block uppercase">Gender</span>
                                 <span className="font-medium text-gray-900 dark:text-zinc-300">{userGender}</span>
                             </div>
                             <div>
                                 <span className="text-xs text-gray-400 block uppercase">Country</span>
                                 <span className="font-medium text-gray-900 dark:text-zinc-300">{userCountry}</span>
                             </div>
                             <div>
                                 <span className="text-xs text-gray-400 block uppercase">City</span>
                                 <span className="font-medium text-gray-900 dark:text-zinc-300">{userCity}</span>
                             </div>
                        </div>
                    </div>
