...
import { 
  LayoutGrid, Users, MapPin, Calendar, Ticket,
  ShieldAlert, FileText, PieChart, Settings, 
  ChevronLeft, ChevronRight, X, Layers, BrainCircuit, LogOut
} from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  const menuItems = [
    ...
    { id: 'moderation', label: 'Moderation', icon: ShieldAlert, href: '/moderation' },
  ...
  return (
    <>
        ...
        <div className={cn(
            ...
            <span className="font-bold text-lg dark:text-white text-slate-800">explore-adama</span>
        ...
        <nav className={cn(
            ...
            return (
                ...
                className={cn(
                    ...
                    isCollapsed ? "justify-center px-0 gap-0 rounded-2xl" : "px-4 gap-4 rounded-2xl",
                    isActive 
                ...
                >
                <Icon size={22} className={cn("shrink-0 transition-colors", isActive ? "text-white" : "group-hover:text-gray-900 dark:group-hover:text-zinc-100")} />
                <span className={cn(
                    "font-medium whitespace-nowrap overflow-hidden transition-all duration-300",
                    isCollapsed ? "max-w-0 opacity-0 -translate-x-4" : "max-w-[200px] opacity-100 translate-x-0"
                )}>
                    {item.label}
                </span>
            ...
            <div className={cn(
                ...
                isCollapsed ? "max-h-0 opacity-0 py-0" : "max-h-8 opacity-100 py-1"
            )}>
            ...
            <div className="space-y-1">
                <a
                    ...
                    className={cn(
                        "w-full flex items-center py-3.5 transition-all duration-300 group relative cursor-pointer",
                        isCollapsed ? "justify-center px-0 gap-0 rounded-2xl" : "px-4 gap-4 rounded-2xl",
                        (activeTab ? activeTab === 'recommendation' : currentPath === '/recommendation')
                        ? "bg-blue-500 text-white shadow-lg shadow-blue-500/30" 
                        : "hover:bg-gray-100 dark:hover:bg-zinc-800 text-gray-500 dark:text-zinc-400 hover:text-gray-900 dark:hover:text-zinc-100"
                    )}
                >
                    <BrainCircuit size={22} className={cn("shrink-0 transition-colors", (activeTab ? activeTab === 'recommendation' : currentPath === '/recommendation') ? "text-white" : "group-hover:text-gray-900 dark:group-hover:text-zinc-100")} />
                    <span className={cn(
                        "font-medium whitespace-nowrap overflow-hidden transition-all duration-300",
                        isCollapsed ? "max-w-0 opacity-0 -translate-x-4" : "max-w-[200px] opacity-100 translate-x-0"
                    )}>
                ...
                <a
                    ...
                    className={cn(
                        "w-full flex items-center py-3.5 transition-all duration-300 group relative cursor-pointer",
                        isCollapsed ? "justify-center px-0 gap-0 rounded-2xl" : "px-4 gap-4 rounded-2xl",
                        (activeTab ? activeTab === 'settings' : currentPath === '/settings')
                        ? "bg-blue-500 text-white shadow-lg shadow-blue-500/30" 
                        : "hover:bg-gray-100 dark:hover:bg-zinc-800 text-gray-500 dark:text-zinc-400 hover:text-gray-900 dark:hover:text-zinc-100"
                    )}
                >
                    <Settings size={22} className={cn("shrink-0 transition-colors", (activeTab ? activeTab === 'settings' : currentPath === '/settings') ? "text-white" : "group-hover:text-gray-900 dark:group-hover:text-zinc-100")} />
                    <span className={cn(
                        "font-medium whitespace-nowrap overflow-hidden transition-all duration-300",
                        isCollapsed ? "max-w-0 opacity-0 -translate-x-4" : "max-w-[200px] opacity-100 translate-x-0"
                    )}>
